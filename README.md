...
|------|------|
| 設備名稱 | 自動填入，唯讀 |
| 設備 IO 點位位址 | 可拖曳下拉填充（智慧遞增：DM0→DM1、DM0.0→DM0.1、MR0→MR1、基恩斯 R000→R001，支援每 16 位元進位 R015→R100） |
| 訊號名稱 | 自由輸入 |
...
- **基恩斯 R 暫存器（Bit 裝置）與三菱 R 暫存器（Word 裝置）動態品牌分類與前導零保留**：通訊中完整保留地址前導零發送，防範 R000 位址位移 Bug。
- **連續地址批次讀取**：同一台設備內相鄰（間距 ≤ 4 words）的 Word 地址自動合併為單一 `RDS` / Batch Read 請求（最多 100 words），大幅減少 TCP round-trip 次數
